...
// Notifications represents a notification record.
type Notifications struct {
	ID        int    `json:"id"`
	UserID    int    `json:"user_id"`
	Message   string `json:"message"`
	Type      string `json:"type"`       // Type of notification (added field)
	CreatedAt string `json:"created_at"` // Creation timestamp (added field)
	Read      bool   `json:"read"`
}
...
func GetUnreadNotifications(db *sql.DB, userID int) ([]Notifications, error) {
